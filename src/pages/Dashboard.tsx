...
export default function Dashboard() {
  ...
  const loadData = async () => {
    ...
    try {
      ...
      // Load certificates for enrolled courses
      ...
      <div className="container max-w-4xl py-6">
        ...
        <div className="grid gap-6 sm:grid-cols-2">
          {courses.map((course, idx) => {
            ...
            const courseLessonsAvailable = hasLessons(course.id);
                ...
                <div className={`px-5 py-4 ${idx === 0 ? "bg-primary/10" : "bg-accent/10"}`}>
                  <div className="flex items-start justify-between">
                    <div className="flex items-center gap-2">
                      <BookOpen className={`h-5 w-5 ${idx === 0 ? "text-primary" : "text-accent"}`} />
                      <span className="text-xs font-medium text-muted-foreground uppercase tracking-wider">
                        {course.duration_weeks} Weeks
                      </span>
                    </div>
                    {cert && <Award className="h-5 w-5 text-accent" />}
                  </div>
